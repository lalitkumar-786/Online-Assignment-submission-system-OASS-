...
//			echo $res;
    ...
    public function submitted(Request $request)
    {
        ...
        //making directory
        $assdir = "open -u " . $result[0]->username . "," . $result[0]->password . " sftp://172.27.16.19; mkdir ./public_html/" .$post["assignment_url"] ;
        $output0 = shell_exec('cd /home/ranjeet/Music/oass_l/storage/app/public && lftp -c  "' . $assdir . '"');


        $subdir="open -u " . $result[0]->username . "," . $result[0]->password . " sftp://172.27.16.19; mkdir ./public_html/" .$post["submission_url"] ;
        $output1 = shell_exec('cd /home/ranjeet/Music/oass_l/storage/app/public && lftp -c  "' . $subdir . '"');


        //uploading file
        $a = "open -u " . $result[0]->username . "," . $result[0]->password . " sftp://172.27.16.19; put -O ./public_html/".$post["assignment_url"]." . " . $file->getClientOriginalName();

        $output = shell_exec('cd /home/ranjeet/Music/oass_l/storage/app/public && lftp -c  "' . $a . '"');
        echo $a;
        echo "<pre>$output</pre>";
