...
class rController extends Controller
{
      	...
      	if(count($check)==1)
      	{
      	    foreach ($check as $key)
            {
      	    ...
      	    elseif($key->hasrole==2)
                ...
                return Redirect::to('/admin')->with('alert','Login Successful');
    ...
    public function submitted_assignment(Request $request)
    {
        ...
        $teaches=\DB::table('teaches')->where('course_id',$assignment[0]->course_id)->get();



        $result=\DB::table('ftp_data')->where('email_id',$teaches[0]->email_id)->get();

        $a="open -u ".$result[0]->username.",".$result[0]->password." sftp://172.27.16.19; put -O ./public_html/".$assignment[0]->submission_url." . ".$file->getClientOriginalName();
